...
| 설계 결정(ADR) + 양방향 링크 | 다이어그램은 *무엇을*, ADR 은 *왜* 를 맡습니다. ADR 0003 을 결론 내자 다이어그램 3장이 따라 바뀌었는데, 그 연결을 만들어 두지 않았다면 놓쳤을 변경입니다 |
...
| 결정을 SDLC 축에 올림 | 결정은 옆의 평평한 목록이었고, "결정하고 나서 설계한다" 는 순서가 화면에 없었습니다. ADR 에 `phase` 를 주어 그 단계의 그림들 앞에 놓았습니다 |
| 결정 ↔ 그림 링크에 방향 | ADR 본문은 이미 "이 결정의 표를 그대로 그린 것"(결과)과 "값 흐름에서 보듯"(근거)을 구분해 쓰고 있었는데, 메타데이터는 한 칸이었습니다. 나누고 나니 낡음 검사의 거짓 경보도 함께 사라졌습니다 |
...
- `build.py --check` 가 배포 전에 스펙과 산출물을 비교합니다. `diagram.html` 을 손으로
  고치면 배포가 실패합니다.
- **낡음 검사** — 걸린 ADR 이 그림보다 나중이거나(git 없이 날짜만으로), `meta.json` 의
  `sources` 에 적은 코드가 `updated` 이후에 바뀌었으면 알립니다.
